...
def buildHull( points ):
    ...
    first = points.pop(0) # leftmost point

    points.sort(key=lambda p: (getSlope(p, first), -p.y,p.x)) # sort rest of points by polar angle to the leftmost point

    result = [first] # leftmost point is on convex hull so start there
    for point in points: # loop through the sorted points
        result.append(point) # add every point we come across to the convex hull
        while len(result) > 2 and turn(result[-3], result[-2], result[-1]) == RIGHT_TURN: # check that the most recent 3 points weve added are in a right turn, 
            # if yes then the middle of the three is wrong and is popped
            result.pop(-2)

    
    points.insert(0, first) # re add the starting point to the global list of points

    for curr,next in zip(result, result[1:]): # zip together each point to its neighbor
        curr.cwPoint=next # set cw and ccw points
        next.ccwPoint=curr
    result[0].ccwPoint = result[len(result) - 1] # wrap around the ccw of the first point
    result[len(result) - 1].cwPoint = result[0] # wrap around the cw of the last point
...
def getSlope(p1, p2): # helper function to get the slope between 2 points
    if p1.x == p2.x:
        return float('inf')
    else:
