...
def buildTristrips( triangles ):
    adjArray = [] # list holding the number of valid adj triangles per triangle
    count = 0 # initial number of strips is 0
    for tri in triangles: # loop through all the triangles
        adjArray.append(numValidAdj(tri)) # add into list the number of valid adj tri strips
    
    sortedTriangles = [x for _, x in sorted(zip(adjArray, triangles), key=lambda pair: pair[0])] # sort the triangles based on the corresponding amount of valid adjs


    for tri in sortedTriangles:
        if (tri.nextTri == None and tri.prevTri == None): # conditions for valid triangle
            count += 1 # starting a new strip so increment count
            findNext(tri) # start a new strip
            sortedTriangles.append(sortedTriangles.pop(sortedTriangles.index(tri))) #move strip to back of the list (not super needed but does visually change the output which is weird)

def findNext(triangle):
    possibleAdj = [] #list for num of valid adjs
    validAdj = [] #list of valid adjs
    for adj in triangle.adjTris:
        if (adj.nextTri == None and adj.prevTri == None): # conditions for valid triangle
            possibleAdj.append(numValidAdj(adj)) # same logic as chosing a starting triangle, look at options of valid adjs, and sort based on the least amount
            validAdj.append(adj)
    sortedValid = [x for _, x in sorted(zip(possibleAdj, validAdj), key=lambda pair: pair[0])] # sort the triangles based on the corresponding amount of valid adjs
    if len(possibleAdj) == 0: # no valid adj triangles, end of the tri strip
        return

    triangle.nextTri = sortedValid[0] # set the next and prev triangles
    sortedValid[0].prevTri = triangle
    findNext(sortedValid[0]) # recursive call to continue the strip



def numValidAdj(triangle): # helper function to get number of valid adj triangles
    numTris = 0
    for adj in triangle.adjTris:
        if(adj.nextTri == None and adj.prevTri == None): # conditions for valid triangle
            numTris += 1
