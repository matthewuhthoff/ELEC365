...
def buildTriangles( slice0, slice1 ):
    ...
    # [YOUR CODE HERE]
    minDist = euclidianDistance(slice0.verts[0], slice1.verts[0]) # brute force solution
    slice0Point = slice0.verts[0]
    slice1Point = slice1.verts[0]
    for vertex0 in slice0.verts:
        for vertex1 in slice1.verts:
            dist = euclidianDistance(vertex0, vertex1) #using helper function to find distance between two points
            if (dist < minDist):
                minDist = dist
                slice0Point = vertex0
                slice1Point = vertex1
    ...
    slice0List = [] # arrays that hold cyclic permutations
    slice1List = [] 
    slice0Start = slice0Point # creating temps so that we can loop through and re add the first at the end again
    slice1Start = slice1Point
    
    for i in range(len(slice0.verts)):  # creates cyclic permutation starting at shortest point found above
        slice0List.append(slice0Point)
        slice0Point = slice0Point.nextV
    slice0List.append(slice0Start) # re add first point to create a closed loop

    for i in range(len(slice1.verts)):
        slice1List.append(slice1Point)
        slice1Point = slice1Point.nextV
    slice1List.append(slice1Start)
    ...
    minArea = [[0 for x in range(len(slice0List))] for y in range(len(slice1List))] # initialize 2d lists with all zeros
    minDir  = [[0 for x in range(len(slice0List))] for y in range(len(slice1List))]


    # Fill in the minArea array

    #minArea[0][0] = 0 # Starting edge has zero area, redundent given the previous 2 lines
    ...
    # [YOUR CODE HERE]
    for i in range(1, len(slice0List)): # start at 1, point at [0][0] stays as 0
        minArea[0][i] = minArea[0][i-1] + triangleArea(slice1List[0].coords, slice0List[i-1].coords, slice0List[i].coords)
        minDir[0][i] = Dir.PREV_COL # always the same
    ...
    # [YOUR CODE HERE]
    for i in range(1, len(slice1List)):
        minArea[i][0] = minArea[i-1][0] + triangleArea(slice0List[0].coords, slice1List[i-1].coords, slice1List[i].coords)
    ...
    # [YOUR CODE HERE]
    for i in range(len(slice1List)):
        for j in range(len(slice0List)): # loop through slice1List x slice0List 2d array
            localMinArea = [(minArea[i-1][j] + triangleArea((slice1List[i-1]).coords, (slice0List[j]).coords, (slice1List[i]).coords)), (minArea[i][j-1] + triangleArea((slice0List[j-1]).coords, (slice1List[i]).coords, (slice0List[j]).coords))]
            minArea[i][j] = min(localMinArea)
            # using the formula from class to find the minArea
            if localMinArea.index(minArea[i][j]) == 0: # if first option then its from PREV_ROW
                minDir[i][j] = Dir.PREV_ROW
            elif localMinArea.index(minArea[i][j]) == 1:
                minDir[i][j] = Dir.PREV_COL

    #for i in range(0, len(slice1List)):
    #    for j in range(0, len(slice0List)):
    #        minArea[i][j] = round(minArea[i][j])
    ...
    nextMinArea = minDir[len(slice1List) - 1][len(slice0List) - 1]
    for i in range(len(slice1List) - 1, 0, -1):
        for j in range(len(slice0List) -1, 0, -1):
            if minDir[i][j] != nextMinArea:
                continue
            elif minDir[i][j] == Dir.PREV_ROW:
                nextMinArea = minDir[i-1][j]
                vertices = [slice0List[j], slice1List[i], slice1List[i-1]]
                tri = Triangle(vertices)
            else:
                nextMinArea = minDir[i][j-1]
                vertices = [slice0List[j], slice1List[i], slice0List[j-1]]
                tri = Triangle(vertices)
...
def euclidianDistance(point1, point2):
    dist = (point2.coords[0] - point1.coords[0]) ** 2 + (point2.coords[1] - point1.coords[1]) ** 2 + (point2.coords[2] - point1.coords[2]) ** 2 # formula for euclidian distance
    dist = math.sqrt(dist)
    return dist
